...
- Prompt 41B added a targeted management-demo receipt enhancement without adding schema, migrations, seed data, or a new business workflow:
  - the existing collection detail endpoint now returns optional customer contact, voucher type/status/date, booking/project/unit, sale contract, and installment context for receipt display
  - `/crm-property-desk/collections/[collectionId]/receipt` provides a read-only browser-printable Customer Collection Receipt for CRM & Property Desk users
  - the collections list and collection detail side panel expose receipt entry points while preserving the existing collection creation and voucher posting flows
  - the receipt reuses the Prompt 40 printable report layout, browser print trigger, screen-only/print-only conventions, and A4 portrait print behavior
- Prompt 41C completed the final receipt demo-readiness pass:
  - `corepack pnpm seed:demo` now refreshes the reserved `real-capita-demo-uat` company before reseeding so stale demo-company records cannot survive into `seed:demo:verify`
  - the synthetic-data verifier remains strict and continues to reject non-synthetic customer/reset-marker drift
  - seeded collection `DEMO-COL-2026-001` now serves as the practical management-demo story for customer -> booking -> contract -> installment -> posted voucher -> collection -> printable receipt
  - runtime receipt QA confirmed table/detail receipt entry points, browser-print output, and shared printable-report regression coverage for the existing financial reports
- Prompt 42B added the Customer 360 Profile + Customer Records + Transaction History enhancement without schema, migration, seed, access, or workflow expansion:
  - `GET /companies/:companyId/customers/:customerId/profile` returns a consolidated read-only profile over customer identity, bookings, sale contracts, installment schedules, collection transaction history, posted voucher context, receipt support, and a minimal business timeline
  - `/crm-property-desk/customers/[customerId]` provides the full customer profile page for existing CRM & Property Desk users only
  - the customer register now exposes a `View Profile` entry point while preserving existing customer create/edit and active-state flows
  - summary metrics are limited to safe counts and totals; broad outstanding, remaining, overdue, documents/attachments, audit-event history, and accountant access remain deferred
- Prompt 42C completed Customer 360 runtime/demo QA and final validation:
  - `DEMO Customer Nadia Synthetic`, behind collection `DEMO-COL-2026-001`, was confirmed as the strong seeded customer profile demo candidate
  - authenticated runtime QA passed for the customer register entry point, full profile sections, transaction-history receipt link, receipt route regression, sparse-customer empty states, and data consistency
...
- CRM & Property Desk receipt:
  - `/crm-property-desk/collections/[collectionId]/receipt`: browser-printable Customer Collection Receipt
