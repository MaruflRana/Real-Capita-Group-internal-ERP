...
- For the seeded supervisor walkthrough, run `corepack pnpm seed:realistic:uat` and `corepack pnpm seed:realistic:verify`.
- `corepack pnpm seed:realistic:uat` refreshes the Real Capita Group company before reseeding, so use it as the authoritative UAT reset before the walkthrough.
...
| 4 min | CRM customer profile/sales/collections | Show customers, open a Customer 360 profile (e.g., search for a Bangladesh-named customer), then show leads, bookings, sale contracts, installment schedules, and collections. Use the profile transaction-history `Printable Receipt` action to show the posted-voucher acknowledgement and browser-print path. | Sales users can access CRM without unrelated admin modules. Customer 360 demonstrates the full chain: customer → booking → contract → installment → posted voucher → collection → printable receipt. |
