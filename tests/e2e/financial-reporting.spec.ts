...
const printableBusinessReportRoutes = [
  {
    path: '/accounting/reports/business-overview',
    title: 'Business Overview Report',
    testId: 'printable-report-business-overview',
    orientation: 'landscape',
  },
  {
    path: '/accounting/reports/daily',
    title: 'Daily Report',
    testId: 'printable-report-daily-report',
    orientation: 'portrait',
  },
  {
    path: '/accounting/reports/weekly',
    title: 'Weekly Report',
    testId: 'printable-report-weekly-report',
    orientation: 'portrait',
  },
  {
    path: '/accounting/reports/monthly',
    title: 'Monthly Report',
    testId: 'printable-report-monthly-report',
    orientation: 'landscape',
  },
  {
    path: '/accounting/reports/yearly',
    title: 'Yearly Report',
    testId: 'printable-report-yearly-report',
    orientation: 'landscape',
  },
] as const;

const expectPrintableBusinessReportRoute = async (
  page: Page,
  {
    orientation,
    path,
    testId,
    title,
  }: (typeof printableBusinessReportRoutes)[number],
) => {
  ...
  await expect(page.getByText('Visual analysis').first()).toBeVisible();
  await expect(page.getByText('Detailed period table')).toBeVisible();

  const printableReport = page.getByTestId(testId);
  await expect(printableReport).toHaveCount(1);
  await expect(printableReport).toHaveAttribute(
  ...
  await expect(page.getByText('Visual analysis').first()).toBeHidden();

  await expect(
    printableReport.getByText('Real Capita ERP', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByRole('heading', {
      exact: true,
      name: title,
    }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Active company', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Report period', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Generated at', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Generated by', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Grouping type', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Data source', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Executive Summary', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Contracted sales').first(),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Collected sales').first(),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Voucher workload', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Period Breakdown', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByRole('columnheader', { name: 'Sale contracts' }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Notes', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText(`Real Capita ERP - ${title}`),
  ).toBeVisible();
};

const printableStatementReportRoutes = [
  {
    path: '/accounting/reports/trial-balance',
    title: 'Trial Balance',
    testId: 'printable-report-trial-balance',
    orientation: 'landscape',
    summaryTitle: 'Debit/Credit Summary',
    tableTitle: 'Trial Balance Statement',
    tableHeader: 'Closing Dr',
  },
  {
    ...
    orientation: 'portrait',
    summaryTitle: 'Revenue And Expense Summary',
    tableTitle: 'Profit And Loss Statement',
    tableHeader: 'Amount',
  },
  {
    ...
    orientation: 'portrait',
    summaryTitle: 'Equation Summary',
    tableTitle: 'Balance Sheet Statement',
    tableHeader: 'Amount',
  },
] as const;

const expectPrintableStatementReportRoute = async (
  page: Page,
  {
    orientation,
    path,
    ...
    testId,
    title,
  }: (typeof printableStatementReportRoutes)[number],
) => {
  await page.emulateMedia({ media: 'screen' });
  await page.goto(path);
  ...
  await expect(
    printableReport.getByText(summaryTitle, { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText(tableTitle, { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByRole('columnheader', { name: tableHeader }).first(),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Notes', { exact: true }),
  ).toBeVisible();
...
test('renders the trial balance page and financial reports navigation', async ({
  ...
  const screenContent = page.getByTestId('printable-report-screen-content');
  await expect(screenContent.getByText('Current Assets').first()).toBeVisible();
  await expect(screenContent.getByText('Petty Cash')).toBeVisible();
  await expect(screenContent.getByText('Report totals')).toBeVisible();
});
...
test('renders business overview and periodic reports from the reporting endpoint', async ({
  ...
  await expect(page.getByText('Yearly buckets').first()).toBeVisible();
  await expect(page.getByRole('button', { name: 'Export CSV' })).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Print Report' }),
  ).toBeVisible();
...
test('renders printable report foundation on business and periodic reports', async ({
  page,
  context,
}) => {
  await context.addCookies([
  ...
  for (const route of printableBusinessReportRoutes) {
    await expectPrintableBusinessReportRoute(page, route);
  }
});

test('renders printable statement reports for trial balance, profit and loss, and balance sheet', async ({
  page,
  context,
}) => {
  await context.addCookies([
  ...
  for (const route of printableStatementReportRoutes) {
    await expectPrintableStatementReportRoute(page, route);
  }
});

test('renders general ledger printable states before and after account selection', async ({
  page,
  context,
}) => {
  await context.addCookies([
  ...
  await expect(
    page.getByRole('heading', { exact: true, name: 'General Ledger' }),
  ).toBeVisible();
  ...
  const printableReport = page.getByTestId('printable-report-general-ledger');
  await expect(printableReport).toHaveCount(1);
  await expect(printableReport).toHaveAttribute(
    'data-print-orientation',
    'landscape',
  ...
  await expect(printableReport).toBeVisible();
  await expect(
    printableReport.getByText('Ledger Summary', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('Ledger Transactions', { exact: true }),
  ).toBeVisible();
  await expect(
    printableReport.getByText('PETTY_CASH - Petty Cash').first(),
  ).toBeVisible();
  await expect(printableReport.getByText('JV-3001 / Journal')).toBeVisible();
  await expect(
    printableReport.getByRole('columnheader', { name: 'Running balance' }),
  ).toBeVisible();
  await expect(page.locator('.app-shell-sidebar')).toBeHidden();
  await expect(page.locator('.app-shell-header')).toBeHidden();
});

test('triggers CSV exports for all printable financial report routes', async ({
  page,
  context,
}) => {
  await context.addCookies([
  ...
  for (const reportPath of exportableReportRoutes) {
    await page.emulateMedia({ media: 'screen' });
  ...
  const ledgerExportButton = page.getByRole('button', { name: 'Export CSV' });
  await expect(ledgerExportButton).toBeVisible();
  await expect(ledgerExportButton).toBeEnabled();

  const ledgerDownloadPromise = page.waitForEvent('download');
  await ledgerExportButton.click();
  const ledgerDownload = await ledgerDownloadPromise;

  await expect(ledgerDownload.failure()).resolves.toBeNull();
  expect(ledgerDownload.suggestedFilename()).toMatch(/\.csv$/);

  await page.emulateMedia({ media: 'print' });
  await expect(ledgerExportButton).toBeHidden();
});
...
test('supports general ledger account selection and surfaces validation and backend errors', async ({
  ...
  const screenContent = page.getByTestId('printable-report-screen-content');
  await expect(
    screenContent.getByText('JV-3001', { exact: true }),
  ).toBeVisible();
  await expect(
    screenContent.getByText('Petty cash replenishment'),
  ).toBeVisible();
  await expect(screenContent.getByText('Voucher ID voucher-1')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Export CSV' })).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Print Report' }),
  ).toBeVisible();
...
test('renders the profit and loss statement', async ({ page, context }) => {
  ...
  const screenContent = page.getByTestId('printable-report-screen-content');
  await expect(screenContent.getByText('Total revenue')).toBeVisible();
  await expect(screenContent.getByText('Operating Revenue')).toBeVisible();
  await expect(screenContent.getByText('Operating Expense')).toBeVisible();
  await expect(
...
test('supports trial balance export and print-ready rendering', async ({
  ...
  await expect(
    page.getByTestId('printable-report-trial-balance'),
  ).toBeVisible();
  await expect(
    page.getByText('Trial Balance Statement', { exact: true }),
  ).toBeVisible();
  await expect(
